...
fn main() {
    ...
    let mut jambe1 = create_triangle_fan(
        &context,
        &[
    ...
    let mut jambe2 = create_triangle_fan(
        &context,
        &[
            vec3(1.0, 1.0, -1.0), // Centre
            vec3(2.0, -2.0, -1.0),
    ...
    let mut jambe3 = create_triangle_fan(
        &context,
        &[
            vec3(-1.0, 1.0, 1.0), // Centre
            vec3(-2.0, -2.0, 1.0),
    ...
    let mut jambe4 = create_triangle_fan(
        &context,
        &[
            vec3(1.0, 1.0, 1.0), // Centre
            vec3(2.0, -2.0, 1.0),
    ...
    let mut epaisseur1 = create_quad_strip(&context, false); // Bleu
    let mut epaisseur2 = create_quad_strip(&context, true);  // Rouge
    ...
    window.render_loop(move |mut frame_input| {
        rotation_angle += 0.01; // Incrémenter l'angle de rotation
        
        // Appliquer la rotation autour de l'axe Y
        let rotation = Mat4::from_angle_y(Rad(rotation_angle));
        jambe1.set_transformation(rotation);
        jambe2.set_transformation(rotation);
        jambe3.set_transformation(rotation);
        jambe4.set_transformation(rotation);
        epaisseur1.set_transformation(rotation);
        epaisseur2.set_transformation(rotation);
        ...
        frame_input
            .screen()
            .clear(ClearState::color_and_depth(0.0, 0.0, 0.0, 1.0, 1.0))  // Noir (R:0, G:0, B:0, Alpha:1)
            .render(
                &camera,
